...
const resolvers = {
  ...
  Query: {
    ...
    // Example usage: query { customer(email) { _id, first_name, lastName, email } }
  ...
  Mutation: {
    addUser: async (parent, { first_name, lastName, email, password }) => {
      const customer = await Customer.create({ first_name, lastName , email, password });
